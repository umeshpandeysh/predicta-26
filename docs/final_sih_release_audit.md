...
- **Pre-Inference Data Quality Gate**: `src/ingestion/data_quality_gate.js` intercepting out-of-bounds inputs ($temp > 175^\circ C$) HTTP 400.
...
| **Pre-Inference Data Quality** | **REAL** | Pre-inference validation gate (`src/ingestion/data_quality_gate.js`). | Rejects malformed/impossible inputs ($temp > 175^\circ C$) HTTP 400. |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **1. NORMAL** | $I_{leak}=110.0\mu A, T=26.0^\circ C, t_{pd}=11.5ns$ | **4.2%** | `PASS` | $3.8\%$ ($\Delta = -0.4\%$ pp) | 🟢 **PASSED** |
| **2. HIGH_LEAKAGE** | $I_{leak}=198.5\mu A, T=36.5^\circ C, t_{pd}=14.8ns$ | **99.9%** | `CRITICAL_FAIL` | $99.5\%$ ($\Delta = -0.4\%$ pp) | 🟢 **PASSED** |
| **3. THERMAL_ANOMALY** | $I_{leak}=175.0\mu A, T=42.0^\circ C, t_{pd}=13.9ns$ | **99.9%** | `CRITICAL_FAIL` | $99.6\%$ ($\Delta = -0.3\%$ pp) | 🟢 **PASSED** |
| **4. TIMING_FAILURE** | $I_{leak}=188.0\mu A, T=38.0^\circ C, t_{pd}=15.2ns$ | **99.9%** | `CRITICAL_FAIL` | $99.6\%$ ($\Delta = -0.3\%$ pp) | 🟢 **PASSED** |
| **5. EQUIPMENT_DRIFT** | $I_{leak}=165.0\mu A, T=32.0^\circ C, t_{pd}=12.8ns$ | **99.9%** | `CRITICAL_FAIL` | $98.2\%$ ($\Delta = -1.7\%$ pp) | 🟢 **PASSED** |
| **6. COMBINED_DEFECT** | $I_{leak}=245.0\mu A, T=48.0^\circ C, t_{pd}=16.5ns$ | **99.9%** | `CRITICAL_FAIL` | $99.8\%$ ($\Delta = -0.1\%$ pp) | 🟢 **PASSED** |
| **7. REVIEW_CASE** | $I_{leak}=110.0\mu A, T=26.0^\circ C, t_{pd}=11.5ns$ | **57.8%** | `SECONDARY_TEST` | $31.2\%$ ($\Delta = -26.6\%$ pp)| 🟢 **PASSED** |

---

## 4. Failure & Security Verification

- **Data Quality Interception**: Inputs with $temp > 175^\circ C$ or missing mandatory fields are intercepted prior to ML execution, returning HTTP 400 `DATA_QUALITY_REJECTED`.
...
$$\mathbf{FINAL\ RELEASE\ VERDICT:\ \ \ \ 🟡\ RELEASE\ READY\ WITH\ KNOWN\ LIMITATIONS}$$
