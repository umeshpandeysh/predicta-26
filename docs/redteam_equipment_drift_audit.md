...
The physical parameter mutations for `EQUIPMENT_DRIFT` were $+15\%$ resistance and $-8\%$ output voltage. These shifts were too small to cross the tree split thresholds designed for severe thermal ($+38^\circ C$) or timing ($+55\%$ delay) defects.
...
2. **Future Engineering Recommendation**: Future dataset iterations should inject equipment-specific sensor offsets (e.g. `EQP-103` temperature reading biased $+5^\circ C$) so that equipment one-hot features can learn true equipment context.
