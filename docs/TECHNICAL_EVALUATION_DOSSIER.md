...
>
> The active model is a **350-tree native XGBoost classifier** trained on the project's **synthetic semiconductor dataset**. Current performance claims must be taken from the active metadata and reproducible tests, not from this archived baseline.
...
4. **Unsupervised Open-Set Layer**: Isolation Forest + PAT/MAD + COPOD catching **up to 94.33% of unseen previously unseen anomalies**.
...
| **Previously Unseen Unseen Anomaly Recall** | **94.33%** | Open-Set Detection | Certified ✅ |
...
* **Why Open-Set Detection?** Standard models force every input into a known training class. PREDICTA's Open-Set layer identifies novel previously unseen defects and routes them to `ENGINEER_REVIEW_FAILURE_ANALYSIS`.
