...
class AnomalyFusionEngineJS {
  ...
  evaluateComponent(features, lotId = null) {
    ...
    const normMad = Math.min(1.0, Math.max(0.0, madRes.score / (this.madDetector ? this.madDetector.rejectZ : 6.0)));
    const normCopod = Math.min(1.0, Math.max(0.0, copodRes.score / (this.copodDetector ? this.copodDetector.rejectScore : 9.5)));
    const normIso = isoRes.score >= 0.40 ? Math.min(1.0, Math.max(0.0, (isoRes.score - 0.40) / 0.30)) : 0.0;
