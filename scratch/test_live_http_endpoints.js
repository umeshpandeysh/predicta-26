/**
 * PREDICTA — Live Local HTTP Endpoint Verification Runner (15 Endpoints)
 * File: scratch/test_live_http_endpoints.js
 */
...
async function runLiveHttpTests() {
  console.log("=========================================================================");
  console.log("PREDICTA PHASE 4 — LIVE HTTP ROUTE VERIFICATION (15 ENDPOINTS)");
  console.log("=========================================================================\n");

  const sampleRecord = {
    test_id: "LIVE-HTTP-001", equipment_id: "EQP-101",
  ...
  const authHeaders = { authorization: "Bearer predicta_op_key_2026", "x-operator-id": "OPERATOR_AZ" };
  ...
  server.listen(PORT, async () => {
    try {
      ...
      const h12 = await request('POST', '/api/prediction/secondary-test/request', { test_id: sampleRecord.test_id, comments: "Live test" }, authHeaders);
      ...
      const sampleDisp = { ...sampleRecord, test_id: "LIVE-HTTP-DISP" };
      ...
      console.error("[FAIL] Live HTTP endpoint test failed:", err.message);
      process.exit(1);
    }
  });
}
